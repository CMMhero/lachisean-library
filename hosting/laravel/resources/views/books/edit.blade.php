    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
              ...
              <div class="p-6 bg-white border-b border-gray-200 w-1/3">
                <form action="/books/{{$book->id}}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div>
                        <x-label for="title" :value="__('Title')" />
                        <x-input id="title" class="block mt-1 w-full" type="text" name="title" value="{{$book->title}}" required />
                    </div>

                    <div class="mt-4">
                        <x-label for="description" :value="__('Description')" />
                        <textarea class="block mt-1 w-full sm:rounded-lg shadow-sm border-gray-300" name="description" required>{{$book->description}}</textarea><br/>
                    </div>

                    <div>
                      <x-label for="category_id" :value="__('Category')" />
                      <select name="category_id" id="category_id" class="block border-gray-300 sm:rounded-lg shadow-sm mt-1 w-full">
                        <option value="{{$book->category_id}}" selected hidden>{{$book->category->name}}</option>
                    ...
                    <div class="mt-4">
                        ...
                        <x-input id="author" class="block mt-1 w-full" type="text" name="author" value="{{$book->author}}" required />
                    </div>

                    <div class="mt-4">
                      <x-label for="cover" :value="__('Update Cover')"/>
